        (function(S,$,D,E) {
            ...
            KISSY.add('zmenu',function(S,_){
                ...
                function Zmenu(klass,tpl,config){
                    ...
                    var self=this;
                    var defaultCfg={
                        dataSource:'http://tms.taobao.com/go/rgn/baobao/test1.php',//数据源,类型为字符串则为异步获取地址,类型为对象则为同步获取对象
                        cache:true,//是否开启缓存
                ...
                S.augment(Zmenu,S.EventTarget,{
                    ...
                    _hoverBind:function(){
                        ...
                        $("body").delegate("mouseenter mouseleave","."+self.klass,function(ev){
                            ...
                            if(ev.type==="mouseenter"){
                                targetBox.attr("data-preleave","false");//预移出标记,改回false来阻止移除active样式
                                if(level>0){
                                    self._hoverPropagate(true,targetBox);
                                }
                            }else{
                                targetBox.attr("data-preleave","true");//预移出标记,如果预定时间内没有被改回false,则移除popup
                                index=targetBox.attr("data-parentIndex");
                                if(level>0){
                                    self._hoverPropagate(false,targetBox);
                                };
                                var target=targetBox.all(".J_hoverList[data-index='"+index+"']");
                        ...
                        $("body").delegate("mouseenter mouseleave", "." + self.klass+ " .J_hoverList",function(ev){
                            ...
                            parentBox=target.parent("."+self.klass),
                            level=+parentBox.attr("data-level"),
                            targetBox=$("."+self.klass+"[data-level='"+(level+1)+"']");
                            if(ev.type==="mouseenter"){
                    ...
                    _preRemovePop:function(target,targetBox){
                        ...
                        S.later(function(){
                            if((!target.hasClass("menuItem_hover"))&&(+targetBox.attr("data-level")!==0)&&(targetBox.attr("data-preleave")!=="false")){
                                self._removePop(targetBox);//移除弹出层
                            }
                    ...
                    _hoverPropagate:function(hover,targetBox){
                        ...
                        if(hover){
                            ...
                            self._preRemovePop(target,targetBox);
                        };
                        //如果还没到根菜单,继续往上冒泡(递归)
                        if(level>0){
